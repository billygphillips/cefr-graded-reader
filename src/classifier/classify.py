...
def should_accept(result, target_level, min_confidence=0.40, min_margin=0.10):
    """
...
_COMPLEXITY_UP = {
    'avg_sent_len', 'avg_word_len', 'out_of_ngsl',
    'subord_ratio', 'relative_clause_rate',
    'passive_rate', 'perfect_rate', 'modal_diversity',
    'conditional_rate', 'avg_tree_depth', 'connector_tier', 'ttr',
}
# Features where higher values = simpler text
_COMPLEXITY_DOWN = {
    'ngsl_800', 'ngsl_1000', 'ngsl_2000', 'ngsl_3000',
}
...
def diagnose(text, target_level, classifier, predicted_level=None, top_n=3):
    ...
    if predicted_level and predicted_level in CEFR_ORDER and target_level in CEFR_ORDER:
        too_complex = CEFR_ORDER.index(predicted_level) >= CEFR_ORDER.index(target_level)
    ...
    for feat, value in features.items():
        ...
        if too_complex:
            ...
            if feat in _COMPLEXITY_UP and value > b["q75"]:
                ...
                deviations.append((severity, feat, value, b["q25"], b["q75"], "too high — simplify"))
            elif feat in _COMPLEXITY_DOWN and value < b["q25"]:
                severity = (b["q25"] - value) / guard
                deviations.append((severity, feat, value, b["q25"], b["q75"], "too low — use simpler words"))
        else:
            # Only flag features pushing toward LESS complexity
            if feat in _COMPLEXITY_UP and value < b["q25"]:
                severity = (b["q25"] - value) / guard
                deviations.append((severity, feat, value, b["q25"], b["q75"], "too low — increase"))
            elif feat in _COMPLEXITY_DOWN and value > b["q75"]:
                severity = (value - b["q75"]) / guard
                deviations.append((severity, feat, value, b["q25"], b["q75"], "too high — use more varied words"))
    ...
    for _, feat, value, q25, q75, hint in top:
        parts.append(f"{feat}={value:.2f} ({target_level} typical: {q25:.2f}–{q75:.2f}, {hint})")
